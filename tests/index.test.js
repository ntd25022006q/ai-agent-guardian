...
describe('src/index.js — Public API', () => {
  ...
  describe('scanFile(filePath)', () => {
    ...
    test('bỏ qua file .test.js (test files được phép có mock)', () => {
      const tmp = path.join(
        os.tmpdir(),
        `guardian-test-${Date.now()}.test.js`
      );
  ...
  describe('runIntegrityCheck(options)', () => {
    test('gọi đủ 4 bước pipeline (Git, Dep, Source, Build) theo thứ tự', () => {
      const calls = [];
      const spies = [
        jest.spyOn(guardian, 'checkGitStatus').mockImplementation(() => calls.push('git')),
        jest.spyOn(guardian, 'checkDependencyGuard').mockImplementation(() => calls.push('dep')),
        jest.spyOn(guardian, 'scanSourceCode').mockImplementation(() => calls.push('src')),
        jest.spyOn(guardian, 'checkBuildSize').mockImplementation(() => calls.push('build')),
      ];
      try {
        runIntegrityCheck();
    ...
    test('truyền options.src qua env MOCK_SRC_PATH cho scanSourceCode', () => {
      const spyScan = jest.spyOn(guardian, 'scanSourceCode').mockImplementation(() => {});
      const spyGit = jest.spyOn(guardian, 'checkGitStatus').mockImplementation(() => {});
      const spyDep = jest.spyOn(guardian, 'checkDependencyGuard').mockImplementation(() => {});
      const spyBuild = jest.spyOn(guardian, 'checkBuildSize').mockImplementation(() => {});
      try {
