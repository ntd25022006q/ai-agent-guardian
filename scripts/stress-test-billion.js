...
function runBillionTest() {
  ...
  for (let i = 0; i < POOL_SIZE; i++) {
    ...
    if (i % 3 === 0) {
      ...
      violatingPool.push(
        `const mySecretApiKey = "ghp_${i}123456789012345678901234567890123456".substring(0, 40);`,
      );
    } else {
  ...
  console.log(
    `   - Tốc độ xử lý: ${(1000 / duration).toFixed(2)} triệu cases/giây`,
  );
  ...
  if (failedCount > 0) {
    ...
    console.log(
      "✅ XÁC MINH TUYỆT ĐỐI: Bộ lọc đạt độ chính xác 100% trên 1 tỷ trường hợp!",
    );
    process.exit(0);
  }
}
