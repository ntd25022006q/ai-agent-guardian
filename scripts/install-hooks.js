...
if (!fs.existsSync(gitDir)) {
  console.error(
    "❌ LỖI: Không tìm thấy thư mục .git. Dự án chưa được khởi tạo Git.",
  );
...
try {
  ...
  console.log(
    `✅ [THÀNH CÔNG] Đã ghi cấu hình bảo vệ tại: ${path.relative(targetProjectDir, preCommitHookPath)}`,
  );
  ...
  try {
    ...
    console.log(
      "✅ [THÀNH CÔNG] Cấp quyền thực thi thành công cho script Git Hook.",
    );
  } catch (chmodError) {
    // Trên Windows chmodSync có thể không đổi được quyền thực thi dạng file system nhưng Git vẫn chạy được.
    console.log(
      "ℹ️  [Lưu ý] Đang chạy trên Windows hoặc hệ thống không hỗ trợ chmod, quyền thực thi Git mặc định đã được thiết lập.",
    );
  }
