...
function checkGitStatus() {
  console.log(
    "🔍 [Guardian] Đang kiểm tra trạng thái Git để phát hiện file bị xóa...",
  );
  try {
    ...
    if (process.env.MOCK_GIT_STATUS !== undefined) {
      ...
      deletedFilesOutput = execSync("git status --porcelain", {
        encoding: "utf8",
      });
    }
    ...
    if (deletedFiles.length > 0) {
      ...
      if (violations.length > 0) {
        console.error(
          `❌ [LỖI NGHIÊM TRỌNG] AI Agent tự ý xóa các file trong thư mục được bảo vệ:`,
          violations,
        );
        console.error(
          `👉 Hãy chạy lệnh 'git restore <file>' để khôi phục lại ngay lập tức.`,
        );
        process.exit(1);
      } else {
        console.log(
          "✅ [Guardian] Không phát hiện file quan trọng nào bị xóa.",
        );
      }
    } else {
    ...
    if (error.message.includes("not a git repository")) {
      console.log(
        "ℹ️  [Guardian] Thư mục hiện tại chưa khởi tạo Git. Bỏ qua kiểm tra Git.",
      );
    } else {
...
function checkDependencyGuard(projectPath = "package.json") {
  ...
  if (!fs.existsSync(projectPath)) {
    console.log(
      "ℹ️  [Guardian] Không tìm thấy package.json. Bỏ qua quét dependencies.",
    );
    return;
  }
  ...
  try {
    ...
    if (violations.length > 0) {
      console.error(
        `❌ [LỖI NGHIÊM TRỌNG] Phát hiện thư viện bị cấm hoặc không an toàn trong package.json:`,
        violations,
      );
      console.error(
        `👉 Yêu cầu AI Agent: Không sử dụng các thư viện đã bị deprecated (như request, node-sass) hoặc thư viện mock trong production.`,
      );
      process.exit(1);
    }
    console.log(
      "✅ [Guardian] Kiểm tra dependencies thành công. Không phát hiện thư viện không an toàn.",
    );
  } catch (error) {
    console.error(
      "❌ [Guardian] Lỗi khi kiểm tra package.json:",
      error.message,
    );
  }
}
...
function scanSourceCode(dirPath = "src") {
  ...
  console.log(
    `🔍 [Guardian] Đang quét tĩnh thư mục '${targetDir}' chống mock data, rò rỉ key và debugger...`,
  );

  if (!fs.existsSync(targetDir)) {
    console.log(
      `ℹ️  [Guardian] Thư mục '${targetDir}' không tồn tại. Bỏ qua quét mã nguồn.`,
    );
    return;
  }
  ...
  try {
    ...
    if (mockViolations.length > 0) {
      console.error(
        `❌ [LỖI NGHIÊM TRỌNG] Phát hiện AI sử dụng dữ liệu MOCK giả lập trong file production:`,
      );
      mockViolations.forEach((v) => {
        console.error(
          `   📍 File: ${path.relative(process.cwd(), v.file)}:${v.line} -> "${v.content}"`,
        );
      });
      hasErrors = true;
    }
    ...
    if (secretViolations.length > 0) {
      console.error(
        `❌ [LỖI AN NINH NGHIÊM TRỌNG] Phát hiện rò rỉ API Keys / Passwords trong code production:`,
      );
      ...
      console.error(
        `👉 Yêu cầu AI Agent: Di chuyển tất cả khóa bí mật vào file .env và import bằng process.env!`,
      );
      hasErrors = true;
    }
    ...
    if (debugViolations.length > 0) {
      console.error(
        `❌ [LỖI CHẤT LƯỢNG] Phát hiện câu lệnh 'debugger;' bị bỏ quên trong file production:`,
      );
    ...
    console.log(
      "✅ [Guardian] Quét mã nguồn an toàn. Không phát hiện vi phạm bảo mật hay dữ liệu mock.",
    );
  } catch (error) {
...
function checkBuildSize(distPath) {
  ...
  console.log(
    `🔍 [Guardian] Đang kiểm tra chất lượng file build trong thư mục '${targetDist}'...`,
  );
  if (!fs.existsSync(targetDist)) {
    console.log(
      `ℹ️  [Guardian] Thư mục '${targetDist}' không tồn tại. Bỏ qua kiểm tra build.`,
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `📦 [Guardian] Tổng dung lượng build: ${totalSizeKB.toFixed(2)} KB (${files.length} tệp tin).`,
    );

    if (totalSizeKB < MIN_BUILD_SIZE_KB && files.length > 0) {
      console.error(
        `❌ [LỖI NGHIÊM TRỌNG] Gói build quá nhẹ (${totalSizeKB.toFixed(2)} KB).`,
      );
      console.error(
        `👉 AI Agent có thể đã thiếu các thư viện cốt lõi hoặc viết code mock sơ sài.`,
      );
      process.exit(1);
    }
    ...
    console.error(
      "❌ [Guardian] Lỗi khi kiểm tra thư mục build:",
      error.message,
    );
  }
}
