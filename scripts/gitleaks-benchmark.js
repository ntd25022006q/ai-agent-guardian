...
const GITLEAKS_BENCHMARK_FIXTURES = [
  ...
  },
  {
    ...
    code: 'const token = "github_pat_' + '1234567890123456789012_1234567890123456789012345678901234567890123456789012345678901234567890123456789012";',
    expected: true
  },
  {
    name: "OpenAI Secret Key (sk-proj)",
    code: 'let openAiKey = "sk-proj-' + '123456789012345678901234567890123456789012345678";',
    expected: true
  },
  {
  ...
  },
  {
    ...
    code: 'const slackBot = "xoxb-' + '123456789012-123456789012-123456789012-12345678901234567890123456789012";',
    expected: true
  },
  {
    name: "Slack User Token (xoxp)",
    code: 'const slackUser = "xoxp-' + '123456789012-123456789012-123456789012-12345678901234567890123456789012";',
    expected: true
  },
  {
...
function runBenchmark() {
  ...
  console.log(`   - Tổng số ca kiểm định: ${GITLEAKS_BENCHMARK_FIXTURES.length}`);
  console.log(`   - Thành công (Passed): ${passed}/${GITLEAKS_BENCHMARK_FIXTURES.length}`);
  console.log(`   - Thất bại (Failed): ${failed}/${GITLEAKS_BENCHMARK_FIXTURES.length}`);
  console.log('==================================================');

  if (failed > 0) {
    process.exit(1);
  } else {
    console.log('🎉 XÁC NHẬN CHINH PHỤC: Bộ lọc đạt tương thích 100% chuẩn Gitleaks!');
    process.exit(0);
  }
}
