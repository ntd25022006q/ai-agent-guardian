...
async function runMonitor() {
  ...
  if (disconnected.length > 0) {
    console.error(
      "⚠️  [CẢNH BÁO] Phát hiện mất kết nối tới một số dịch vụ API AI.",
    );
    console.error(
      "👉 Khuyến nghị: Kiểm tra lại mạng, Proxy VPN hoặc VPS của bạn.",
    );
    console.error(
      "👉 Tránh tiếp tục chat với AI lúc này để hạn chế lỗi nghẽn lệnh.",
    );
    process.exit(1);
  } else {
